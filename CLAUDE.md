...
Переведены на новую архитектуру: **message_classifier, verdict_classifier, extractor_agent, one_line_search_query_builder, sourcing_assistant, responsibilities_parser, screening_autofill, first_touch (base)**.
Остальные (screening_scenarios(+hh), screening_guardrails, first_touch_hh/_event)
— пока в `app/`.
