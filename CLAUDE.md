...
Четыре траектории: **A** всё отвечает → `FINISH`+нули · **B** «нет опыта» по ОДНОМУ навыку → не отсев ·
**C** «всё в резюме» → переспрос→`refused`→дальше, без STOP · **D** требует вилку → `STOP_SALARY_DEMAND`
(не `KO_SALARY`). Отчёт — два файла (review.md отключён, как у split; трасса хода лежит на самой реплике
в `transcript`). Ядро **только `policy`** (флага нет: пороги кейсов выведены из его `budgets.py`, на split
тот же прогон ассертил бы не то), Аналитик по умолчанию **v3** — иначе ядру нечего читать. `--offline` —
