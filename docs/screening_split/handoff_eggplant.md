...
**Миграция существующих строк.** Прод работает, диалоги, начатые старым движком, придут в новое ядро:

1. **вилку разобрать из строки `context` ДО её вырезания.** Формат строки — тот же, что в tg:
   `Зарплатная вилка: от X до Y рублей (НЕ РАСКРЫВАТЬ!)` (`context.py::salary_display`). Порядок
   обязателен, и цена ошибки скрытая: без вилки `compare_with_band` всегда вернёт «проходит», то есть
   **зарплатный отсев отключится ТИХО**;
2. **ключи `state` добить дефолтами** — это JSONB, делается КОДОМ при загрузке, Alembic не нужен;
3. **`last_asking` обнулить**: у поля сменился автор (писала модель, станет писать код). Сравнение
   нового фокуса со старым значением начислит переспрос, которого не было;
4. в tg миграция ленивая (при первой загрузке) + `schema: 2` пишется при создании — можно повторить
   ровно так же, ключ внутри JSONB.

**Alembic-ревизия нужна ровно для одного** — если типизированную вилку решим хранить КОЛОНКОЙ.
Собирать её на лету из `hh_vacancy_data["salary"]` ревизии не требует. **Решение открыто** — см. §9.
...
1. **Типизированная вилка: колонка или на лету.** Колонка в `screening_dialogues` → Alembic-ревизия,
   зато вилка не зависит от вакансии после старта диалога. Сборка на лету из
   `hh_vacancy_data["salary"]` → ревизии нет, но чтение `currency` обязано жить там, и вилка может
   поменяться под уже идущим диалогом. Ядро читает вилку из одного места (`DecideContext`), поэтому
   решение локальное. **Не принято — спросить владельца задачи.**
2. **Комментарии и докстринги** в файлах порта: в tg-PR их сняли по требованию владельца репо, для
   eggplant решать отдельно.
3. **Куда писать аудит зарплатного claim** (в tg — коллекция Mongo).
4. **Б3, кандидаты за границей.** В hh остался рудимент: `KO_LOCATION_GEO` отсеивает только при
...
1. ядро в `eggplant-api` — `policy/` + зарплатный модуль + async-обвязка; гарды внутри движка
   ...
   шлюз стоит в движке);
2. хранилище — решение №1 выше, при необходимости Alembic-ревизия;
