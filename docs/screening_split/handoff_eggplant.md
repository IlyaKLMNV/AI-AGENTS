...
6. В репозитории лежит untracked-файл `docker-compose.override.yml` — не наш, в коммиты не берём.
...
| `rules.py` | 138 | таблица правил R1–R11, первое сработавшее выигрывает | R5 `KO_LOCATION_GEO` · **R5a `KO_LOCATION`** (отказ переезжать; в tg ключ заведён вместе с Р18) · R6 `KO_FORMAT` по мультиформату, от локации не зависит · R10 без ветки источника контакта |
...
поля, промпт без кода отдавал бы наблюдение в пустоту. **Тела v3 правим на месте, пока прод на них не
работал** (Р19): 01.09 так исправлено смешение формата и локации в обоих промптах. Следствие — нужен
релиз `prompts` 1.2.3: в 1.2.2 лежит текст ДО правки.
...
| `greeted` | приветствие приклеено к первому сообщению и больше не повторится | есть в обоих: механика заведена в hh-ядре харнесса (Р16) и гейтится инвариантом `greeting_once` |
...
| приветствие | нет | `greeting` приклеивается к первому сообщению, флаг `state.greeted`. В hh-ядре харнесса механика есть (Р16): склейка **только на ходе Интервьюера**, на ход-скрипт не клеится, текст входит в `last_sent` |
...
- приветствие: приклеено к первому сообщению Интервьюера, не повторяется, на ход-скрипт не клеится
  (в харнессе это инвариант `greeting_once`, четыре ветки провала — Р16).

**Анти-луп канала уже заведён в харнессе** — `tests/fixtures/screening_split_hh/counter_loops.yaml`
(8 кейсов: четыре лупа, два канальных — на формате и разъездном, два кооперативных негативных) плюс
`--channel hh` у раннера `screening_counters`. Это готовый эталон того, что должны
проверять юнит-тесты порта: настойчивый переспрашиватель завершается в пределах капа, кооперативный
доходит до `FINISH` без ложного срабатывания.
...
Первые две печатают `[offline] hh-ядро policy … 33/33` и `[dialogue] passed=4/4 · errors=0 ·
...
1. **Куда писать аудит зарплатного claim** (в tg — коллекция Mongo).

**Закрыто, делать как в tgApi:**

- **комментарии и докстринги в файлах порта — снять**, как в tg-PR (осталась только прагма
  `# noqa: BLE001`). Плотность комментариев в остальных файлах `eggplant-api` не трогаем;
- **гео-отсев (бывший Б3) — политика tg** (Р17): безусловного отсева кандидатов за границей нет,
  остаётся условный — вакансия ЯВНО прописала гео-ограничение И Наблюдатель увидел нарушение.
  Механизм в каналах уже идентичен (`rules.r5_geo_ko`), различаются только ключ и текст: tg
  `KO_GEO`, hh `KO_LOCATION_GEO`. **Ключ не переименовываем** — он канальный и вшит в реестр старого
  движка, шесть фикстур и сценарий 12; оба CODE-авторства, промпт их не эмитит;
- **приветствие** (Р16) — механика уже есть в hh-ядре харнесса, переносится вместе с ядром: склейка
  только на ходе Интервьюера, ровно один раз, флаг `greeted` ведёт код, текст входит в `last_sent`.
...
**Приёмка:** их pytest зелёный · офлайн-гейт харнесса 33/33 и `screening_dialogue --channel hh
