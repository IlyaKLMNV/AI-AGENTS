# Легаси-раннеры `app/` — справочник (до cutover)

> **Статус: заморожено.** Миграция раннеров на `qa_harness` **завершена** — для любой новой работы
> берите новые раннеры (`python -m qa_harness.runners.<name>`, см. [README](../README.md)).
> Это дерево остаётся рабочим только до cutover, потому что на него ещё опираются `adapters/`
> и `screeningAssistant/screeningAss.py`, и удалять его рано. Новый код сюда не добавляем
> (контракт `qa_harness ⊥ app` проверяет `lint-imports`).
>
> Отчёты этих раннеров пишутся в `tests/reports/<runner>/` (монолитный JSON, схема
> [REPORT_SCHEMA.md](REPORT_SCHEMA.md) на них НЕ распространяется).
...
### Не описаны отдельно
Тот же контракт, что у одноимённых базовых раннеров, отличается только каналом/хелперской ролью:
- `app/first_touch_hh_runner.py` — hh-вариант `first_touch_runner.py`;
- `app/screening_scenarios_hh_runner.py` — hh-вариант `screening_scenarios_runner.py`
  (CSV `tests/fixtures/screening_scenarios_hh.csv`);
- `app/screening_autofill_client.py` — клиент-хелпер для `screening_autofill_runner.py`, не раннер.
