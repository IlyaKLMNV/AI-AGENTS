...
def load_cdm_files(cdm_dir: Path, cdm_count: Optional[int]) -> List[Path]:
...
def _run_synthetic(rb, generator, classifier, judge, args, clf_bucket, results, quiet, seed) -> None:
    ...
    for target in CLASSES:
        ...
        while got < need and attempts < attempts_limit:
            ...
            try:
                message = generator.generate(MessageSpec(_load_json(cdm_path), target, hint, args.noise_level))
