...
DEFAULT_FIXTURE = REPO_ROOT / "tests" / "fixtures" / "screening_split" / "dialogue_cases.yaml"
...
def _vacancy_for(case: Dict[str, Any], defaults: Dict[str, Any]) -> Dict[str, Any]:
    """Вакансия кейса: общий дефолт ← блок `vacancy` фикстуры ← пер-кейсовые оверрайды."""
    vac = {**DEFAULT_VACANCY, **(defaults or {})}
    for field_name in ("work_format", "location", "questions", "company_name",
...
_OFFLINE_BEHAVIOUR = {"A": "cooperative", "C": "refuser", "D": "demander"}
...
class _ScriptedObserver:
    ...
    def __init__(self, behaviour: str, city: str = "Москва") -> None:
...
class _OfflineConversation:
    ...
    def __init__(self, vacancy_info: Dict[str, Any], behaviour: str) -> None:
        from qa_harness.domain.screening_split.policy.engine import PolicyEngine

        self._engine = PolicyEngine(sp.InMemoryStateStore(), _ScriptedObserver(behaviour),
    ...
    def respond(self, message: str) -> Any:
        ...
        state = {"salary": st.get("salary"), "format_check": st.get("format_check"),
                 ...
                 "counters": dict(st.get("counters", {}))} if st else None
...
def _run_case(case: Dict[str, Any], vacancy: Dict[str, Any], *, offline: bool, max_turns: int,
    ...
    key = case.get("key", "?")
    if offline:
        conv: Any = _OfflineConversation(vacancy, _OFFLINE_BEHAVIOUR.get(key, "cooperative"))
...
def run(args: argparse.Namespace) -> None:
    ...
    if args.case != "all":
        ...
        if not cases:
            raise SystemExit(f"кейса {args.case} нет в {args.fixture}")
    ...
    setup: Dict[str, Any] = {}
    models: Dict[str, Any] = {}
    put: Dict[str, Any] = {"component": RUNNER, "source": "local"}
    ...
    if args.offline:
        ...
        print(f"[dialogue] ОФЛАЙН · ядро {ENGINE} на заглушках · кейсов: {len(cases)}")
    else:
        ...
        setup = dict(client=client, analyzer_client=analyzer_client, interviewer_spec=interviewer_spec,
        ...
        print(f"[dialogue] Аналитик {a_spec.version}/{a_spec.model} · Интервьюер "
    ...
    rb = ReportBuilder(runner=RUNNER, prompt_under_test=put, run_id=run_id,
                       started_at=started.isoformat(timespec="seconds"), models=models,
                       seed=args.seed,
                       args={"engine": ENGINE, "offline": bool(args.offline),
                             "max_turns": args.max_turns, "cases": len(cases)})

    total_usage = blank_usage()
    for case in cases:
        vacancy = _vacancy_for(case, fixture.get("vacancy") or {})
    ...
    metrics_path, _ = write_reports(args.out_dir, RUNNER, run_id, metrics_doc, cases_doc,
                                    write_review=False)
...
def build_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(description="Тест нормальных диалогов: доигранная повестка и итог (A/B/C/D).")
    p.add_argument("--case", choices=["A", "B", "C", "D", "all"], default="all")
    ...
    p.add_argument("--analyzer-version", default=ANALYZER_VERSION, metavar="vN",
                   help=f"Версия {ANALYZER_COMPONENT} (по умолч. {ANALYZER_VERSION} — её требует ядро).")
    p.add_argument("--interviewer-version", default=None, metavar="vN",
                   help=f"Версия {INTERVIEWER_COMPONENT} (по умолч. pointer.yaml active).")
