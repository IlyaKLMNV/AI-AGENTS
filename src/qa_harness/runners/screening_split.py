...
# Пер-сценарный контекст вакансии и констрейнты генерации — СВОИ, ключёванные по индексам этого
# CSV. Раньше брался набор монолита, и его индексы совпадали со split только случайно: любая правка
# нумерации молча отвязывала записи, сценарий получал DEFAULT_VACANCY_INFO, и проверка (гео-отсев,
# скрытый поиск, формат) переставала проверять то, ради чего заведена.
DEFAULT_VACANCIES = FIXTURES / "generation" / "screening_split" / "scenario_vacancies.yaml"
DEFAULT_CONSTRAINTS = FIXTURES / "generation" / "screening_split" / "constraints.yaml"
...
def _fixture_checks(scenarios, checks_by_index, inputs_by_index, vacancies, constraints_entries):
    """[(имя, ok, деталь)] — связность пяти фикстур канала.

    Индексы живут в пяти файлах, и расхождение между ними ТИХОЕ: сценарий просто возьмёт
    DEFAULT_VACANCY_INFO вместо своей вакансии, и проверка станет проверять не то, ради чего
    заведена (гео-отсев, скрытый поиск, формат). Раньше это ловилось только чтением глазами.
    """
    out = []
    idx = [s.index for s in scenarios]
    known = set(idx)

    out.append(("нумерация CSV сплошная 1..N", idx == list(range(1, len(idx) + 1)),
                f"{idx[:3]}...{idx[-3:]}" if idx else "пусто"))
    no_checks = sorted(known - set(checks_by_index))
    out.append(("у каждого сценария есть инвариант", not no_checks, str(no_checks)))
    no_recipe = sorted(known - set(inputs_by_index))
    out.append(("у каждого сценария есть скриптовый вход", not no_recipe, str(no_recipe)))
    for name, extra in (("инварианты", set(checks_by_index) - known),
                        ("рецепты", set(inputs_by_index) - known),
                        ("вакансии", set(vacancies) - known)):
        out.append((f"{name}: нет записей вне CSV", not extra, str(sorted(extra))))
    # `load_constraints` отдаёт словари, а не объекты: читаем ключом, иначе проверка была бы пустой.
    cons_idx = {c.get("index") for c in constraints_entries if isinstance(c, dict) and c.get("index")}
    ...
    for scenario in scenarios:
        ...
        if not recipe:
            continue
        vinfo = vacancy_for(scenario, vacancies, DEFAULT_VACANCY_INFO)
        turns = sp.build_scripted_turns(recipe, vinfo, variant=0, seed=0, index=scenario.index)
        if not turns or any("{" in t for t in turns):
            unresolved.append(scenario.index)
    out.append(("все рецепты разворачиваются без остатка", not unresolved, str(unresolved)))
    return out


def _selfcheck_suites():
    """Все наборы офлайн-гейта: ядро ОБОИХ каналов, независимо от `--channel`.

    Канал выбирает фикстуры, а не код: `policy/` обоих каналов лежит в одном репозитории, и
    регрессия в канало-независимой части обязана краснеть в любом прогоне. Раньше `--channel hh`
    гонял только hh-дельту, и правку общего ядра приходилось ловить вторым прогоном.
    """
    from qa_harness.domain.screening_split.selfcheck import SUITES as TG_SUITES
    from qa_harness.domain.screening_split_hh.selfcheck import SUITES as HH_SUITES
    return ([("tg", title, fn) for title, fn in TG_SUITES]
            + [("hh", title, fn) for title, fn in HH_SUITES])


def _run_offline(args: argparse.Namespace, scenarios: List[Scenario], channel: str,
                 fixture_rows: List[tuple]) -> int:
    """Плумбинг фикстур + офлайн-гейт ядра. Возвращает число провалов (0 = всё зелено)."""
    with_ex = turns = 0
    for s in scenarios:
    ...
    print("")
    print(f"[offline] сценариев={len(scenarios)} с_примерами={with_ex} реплик_всего={turns}"
          f" (плумбинг фикстур канала {channel}, без судьи)")

    failed = total = 0
    for title, rows in (("связность фикстур канала", fixture_rows),):
        bad = sum(1 for _, ok, _ in rows if not ok)
        failed += bad
        total += len(rows)
        print(f"[offline] {channel} · {title}: {len(rows) - bad}/{len(rows)}")
        for name, ok, detail in rows:
            if not ok or not args.quiet:
                print(f"  {'OK  ' if ok else 'FAIL'} {name}" + (f" — {detail}" if detail else ""))
    for tag, title, run_suite in _selfcheck_suites():
        rows = run_suite()
        bad = sum(1 for _, ok, _ in rows if not ok)
        failed += bad
        total += len(rows)
        print(f"[offline] {tag} · {title}: {len(rows) - bad}/{len(rows)}")
        for name, ok, detail in rows:
            if not ok or not args.quiet:
                print(f"  {'OK  ' if ok else 'FAIL'} {name}" + (f" — {detail}" if detail else ""))
    print(f"[offline] гейт ядра: {total - failed}/{total} · сеть и пакет prompts не дёргались.")
    return failed


def _resolve_version(cfg: Dict[str, Any], component: str, cli_version: str | None,
                     fallback: str | None = None) -> str | None:
    """CLI > model.yaml[component].local_version > fallback > None (pointer.yaml active в пакете)."""
    if cli_version:
        return cli_version
    return component_cfg(cfg, component).get("local_version") or fallback
...
def run(args: argparse.Namespace) -> Any:
    ...
    if args.offline:
        ...
        # Связность фикстур считается по ПОЛНОМУ набору, а не по выборке: расхождение индексов —
        # свойство файлов, а не тех пяти сценариев, что попали в --sample.
        fixture_rows = _fixture_checks(scenarios, sp.load_checks(checks_path),
                                       sp.load_candidate_inputs(inputs_path),
                                       load_vacancies(vacancies_path),
                                       load_constraints(constraints_path))
        failed = _run_offline(args, selected, channel, fixture_rows)
        if failed:
            ...
            raise SystemExit(f"[offline] провалено проверок: {failed}")
        return None
    ...
    a_ver = _resolve_version(cfg, analyzer_component, args.analyzer_version, ANALYZER_VERSION)
