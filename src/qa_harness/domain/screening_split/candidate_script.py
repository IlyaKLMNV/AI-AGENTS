...
_SALARY_DIRECTIVE = {
    # ── величина суммы (сумма подставляется из вилки вакансии) ──
    ...
    # ── ФОРМА записи: код обязан привести её к рублям за месяц ──
    # `ambiguous` = голое число без единиц. В tg-канале это НЕ неоднозначность (код достраивает
    # масштаб), а в hh-канале промпт по-прежнему требует уточнения — категория общая, поэтому имя
    # оставлено как есть: инвариант у каналов разный, директива генератору одна и та же.
    ...
    "threshold_words": "сформулируй ожидания порогом СЛОВАМИ, без слова «от»: «ниже {above_max} рублей "
                       "не рассматриваю»",
    "conditional": "назови сумму {in_band} рублей внутри условной фразы: согласен на неё, ЕСЛИ будет "
                   "квартальный бонус 15%",
    # ── ЧЬЯ сумма и ожидание ли это: классы, на которых старый разбор давал ложный вердикт ──
    "third_party": "сошлись на зарплату КОЛЛЕГИ на похожей позиции — {above_max} рублей у него; свои "
                   "ожидания НЕ называй вообще",
    "own_current": "назови ТОЛЬКО свою текущую зарплату {below_min} рублей и скажи, что хочешь больше; "
                   "конкретных ожиданий НЕ называй",
    "current_plus_expected": "назови сначала свою ТЕКУЩУЮ зарплату {in_band} рублей, а сразу за ней "
                             "ОЖИДАНИЯ {above_max} рублей — именно в таком порядке",
    "two_own_amounts": "назови желаемую сумму {above_max} рублей и сразу добавь, что готов рассмотреть "
                       "и меньшую — от {in_band} рублей",
    "relative_only": "попроси прибавку в ПРОЦЕНТАХ к текущей зарплате («плюс 15%») и не называй "
                     "никакой абсолютной суммы",
    "not_stated": "скажи, что сумма обсуждаема и зависит от задач; никаких чисел НЕ называй",
    "currency_unknown": "назови сумму в ФУНТАХ — валюте, курса которой у нас нет: «от 4000 фунтов "
                        "в месяц»",
}
