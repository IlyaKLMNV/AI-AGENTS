...
def r9_agenda_complete(f) -> Optional[Outcome]:
    ...
    ПАУЗА ЭТО ПРАВИЛО НЕ УДЕРЖИВАЕТ (решение 28.08.2026, закрывает `backlog.md` §4). Кандидат
    отвечает на последний незакрытый вопрос и тем же сообщением говорит «буду ждать звонка» — всё
    собрано, значит финиш. Промпт противоречил сам себе: `system.md:263` давал паузе приоритет,
    `system.md:345` требовал немедленного `FINISH`; побеждает второе, и `:263` в v3 правится.
