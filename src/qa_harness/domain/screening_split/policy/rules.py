...
(прежнем движке), здесь нет и быть не может: правило либо сработало, либо нет.
...
Три сегодняшних решения, растворённых в порядке строк (проверяются офлайн-переигрыванием):
    ...
    R4 > R7  — зарплатный вердикт выше событийных порогов         
...
def r1_dialogue_closed(f) -> Optional[Outcome]:
    """Диалог уже закрыт — молчим и остаёмся закрытыми, а не переоткрываем."""
...
def r4_salary_ko(f) -> Optional[Outcome]:
    ...
    поднявший ожидания в середине диалога, должен быть отсеян (порт прежнем движке).
    """
...
RULES: tuple[Rule, ...] = (
    ...
    # где он перебивает любое нетерминальное решение, но не трогает терминальное.
