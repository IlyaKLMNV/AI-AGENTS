...
_KO = {
    ...
    "KO_FORMAT_OFFICE": Reason(
        ...
        "так как предполагается офисный формат работы. В любом случае, спасибо за уделенное время!",
        True, CODE),
    "KO_FORMAT_HYBRID": Reason(
        "Поняла вас, спасибо. К сожалению, для этой позиции важно находиться в городе {city}, "
        "так как предполагается гибридный формат работы. В любом случае, спасибо за уделенное время!",
        True, CODE),
    "KO_FORMAT_NOCITY": Reason(
        "Поняла вас, спасибо. К сожалению, для этой позиции важна фиксированная локация из-за "
        "формата работы. В любом случае, спасибо за уделенное время!", True, CODE),
    "KO_GEO": Reason(
...
_STOP_BY_SIGNAL = {
    ...
    "STOP_POLITICS": Reason(
        "Прошу прощения, обсуждение таких тем я вести не могу. Спасибо за уделённое время!", True, MODEL),
