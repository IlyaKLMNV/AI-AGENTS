...
рендерятся в дословно одинаковое «Прошу прощения за беспокойство», поэтому по
...
Три проверки выполняются ПРИ ИМПОРТЕ и роняют процесс на старте, а не в диалоге с кандидатом:
  ...
  1. текст непустой — чинит `STOP_POLITICS`, который сегодня рендерится в пустую строку
     и кандидат не получает вообще ничего;
  2. у каждого кода есть производитель — правило, строка бюджета или сигнал. `STOP_ABROAD`
     до релиза не доживает: промпт его больше не эмитит, но `is_known` его
...
себя. Стало — подстановка по умолчанию.
"""
...
_STOP_BY_BUDGET = {
    ...
    # Прежний текст обещал «сама напишу вам позже» — обещание, которое код не выполняет:
    # STOP_PAUSE терминален, никто кандидату потом не пишет. Текст общий с прочими стопами (tgApi 24fca71).
...
def is_terminal(reason_code: str) -> bool:
    ...
    считается по «всё, что не REPLY_*», и любой новый код молча становится
