...
Что было в прежнем движке — ДВЕ разные семантики:
...
# ── событийные счётчики (сегодня `_EVENT_STOP` в прежнем движке) ────────────
# `salary_info` порога не имеет намеренно (решения Д4 и О4 не переоткрываются). Ход диалога он не
# определяет: ступень переспроса задаёт `salary_reasks`, а `salary_info` лишь ВЫБИРАЕТ объяснение —
# кандидат требовал нашу вилку («назвать не можем, нужен ваш ориентир») или просто не назвал сумму
# («сверить с бюджетом»). Это разные события: молчун даёт переспросы без `salary_info`, а требователь
# вилки — `salary_info` без единого переспроса; поэтому счётчик остаётся и как след в трассе.
...
# ── лимит переспросов одного и того же незакрытого пункта ─
