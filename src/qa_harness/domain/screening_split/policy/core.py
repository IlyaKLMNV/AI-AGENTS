...
_SIGNAL_CONVEY: dict[str, str] = {
    ...
    # Созвон, встреча и голосовые — один и тот же запрос «уйдём из чата», и ответ у них общий.
    # Причина названа кандидату не для вежливости: без неё отказ выглядит произволом. Формулировка
    # взята из монолитного промпта, где этот ответ был прописан дословно.
    "scheduling": ("Откажись от созвона, встречи и голосовых сообщений: первичный скрининг ты ведёшь "
                   "только здесь, в чате, и только текстом — так ответы можно проверить и передать "
                   "коллегам."),
