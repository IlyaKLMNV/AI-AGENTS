...
def _ask_slot(focus: str, state: dict, ctx: DecideContext) -> str:
    ...
    """
    if focus == "salary":
        if state.get("counters", {}).get("salary_info", 0) >= 1:
            return ("Переспроси, на какую сумму на руки в месяц ориентируется кандидат. "
                    "Ничего не объясняй и не добавляй вводных фраз — только сам вопрос.")
    ...
    if focus == "format":
        ...
        if not state.get("candidate_city"):
            return (f"Донеси: формат — {human}" + (f" в городе {city}" if city else "") +
                    ", это обязательное требование вакансии. Спроси, в каком городе находится кандидат "
                    "и готов ли он работать в таком формате.")
        return (f"Донеси: формат — {human}" + (f" в городе {city}" if city else "") +
                ", это обязательное требование вакансии. Спроси, готов ли кандидат работать в таком формате.")
    ...
    return f"Задай дополнительный вопрос по теме: «{question['text']}»."
...
def _convey_slot(obs: Observation, state: dict, focus: Optional[str]) -> str:
    ...
    ВАЖНО: две директивы кода не должны противоречить друг другу. Шаблон переспроса зарплаты прямо
    требует «ничего не объясняй, только сам вопрос», поэтому пояснительный convey рядом с ним
    запрещён — иначе инструкция читается как «объясни и не объясняй» (прогон 28.08, сценарий 29).
    """
...
def _build_instruction(obs: Observation, outcome: Outcome, state: dict,
                       ctx: DecideContext) -> tuple[str, list[dict]]:
    """`instruction` = [черновик модели] + [вопрос из шаблона кода].
...
def decide(state: dict, observation: Observation, message: str, ctx: DecideContext, *,
    ...
    if outcome.reason_code == REFUSE_AND_ADVANCE:
