...
Реестр канальный (тексты и набор ключей — из `..scripts`), проверки те же три, что в TG, и работают
...
_STOP_BY_SIGNAL = {
    # `STOP_POLITICS` в `..scripts` рендерится пустой строкой — кандидат не получает ничего.
